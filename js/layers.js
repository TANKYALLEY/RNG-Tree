	...
	for (let r=1;r<=RNG_DATA.rows;r++) {
		...
		for (let l=1;l<=layersInRow;l++) {
			...
			if (hasUpgrades) {
				...
				layerInfo.upgrades = {
					rows: Math.floor(random(rand*seed)*Math.min(2*(layerInfo.row)+1, 1000)+1),
					cols: Math.floor(random((1-rand)*seed)*Math.min(2*(layerInfo.row+1), 9)+1),
				}
					...
					for (let upgCol=1;upgCol<=layerInfo.upgrades.cols;upgCol++) {
						...
						layerInfo.upgrades[id] = {
							...
							effect() { 
								...
								eff = new Decimal(amt||0).max(1).plus(1).mul(5)
